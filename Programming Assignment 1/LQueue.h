...
class Queue
{
 public:
  ...
  -----------------------------------------------------------------------*/
void move_to_front(const QueueElement& keyValue);
  /*-----------------------------------------------------------------------
    Retrieve value equivalent to keyValue in the queue, and move it to the front
    of the queue.
   
    Precondition: None
    Postcondition: If a value equal to keyValue is found in the queue, the
        value is moved to front of the queue; otherwise, a message is printed
        stating keyValue was not found in the queue.
  -----------------------------------------------------------------------*/
void merge_two_queues(Queue& other);
 /*-----------------------------------------------------------------------
        Merges sorted queue with another sorted queue. Puts all items in the
        first queue and empties the second queue, maintaining the order
        of the items in the merged queue.
       
        Precondition: None
        Postcondition: First queue contains every item from both queues, and
