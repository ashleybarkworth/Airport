...
void Queue::move_to_front(const QueueElement& value) {
    if(!empty()) {
       ...
       if(myFront->data == value) {
          ...
          while(prev!=0) {
             if(curr->data == value) {
                Queue::NodePointer ptr = curr;
                Queue::NodePointer front = new Queue::Node(curr->data);
                front->next = myFront;
                myFront = front;
                prev->next = curr->next;
                delete ptr;
       ...
       } cerr << "*** keyValue not found -- can't move a value to the front ***\n";
    } else {
      cerr << "*** Queue is empty -- can't move a value to the front ***\n";
    }
 }
...
void Queue::merge_two_queues(Queue& other) {
   ...
   while(!empty() && !other.empty()) {
      if(myFront->data < other.front()) {
        q3.enqueue(front());
        dequeue();
      } else {
        q3.enqueue(other.front());
        other.dequeue();
     }
   }
   while(!empty()) {
